...
def greatest_product_in_grid(grid):
    ...
    # Function to calculate product in a given direction
    def product_in_direction(x, y, dx, dy):
        product = 1
        for i in range(4):
    ...
    for y in range(rows):
        for x in range(cols):
            # Check horizontally
            if x + 3 < cols:
                max_product = max(max_product, product_in_direction(x, y, 1, 0))
            # Check vertically
            if y + 3 < rows:
                max_product = max(max_product, product_in_direction(x, y, 0, 1))
            # Check diagonal (top-left to bottom-right)
            if x + 3 < cols and y + 3 < rows:
                max_product = max(max_product, product_in_direction(x, y, 1, 1))
...
# Compute the result
result = greatest_product_in_grid(grid)
print("The greatest product of four adjacent numbers in the grid is:", result)
